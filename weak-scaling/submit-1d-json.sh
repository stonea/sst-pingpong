      ...
      for componentCount in $componentCounts; do
        for messageCount in $messageCounts; do
          for timeStepCount in $timeStepCounts; do
            outfile=${nodeCount}_${tasksPerNode}_${threadCount}_${componentCount}_${messageCount}_${timeStepCount}_json_1d.time
            sbatch -N $nodeCount --cpus-per-task $threadCount --ntasks-per-node $tasksPerNode -o $outfile scaling-1d-json.sh $nodeCount $tasksPerNode $threadCount $componentCount $messageCount $timeStepCount
          done
        done
      done
    done
