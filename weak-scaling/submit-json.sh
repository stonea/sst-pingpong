      ...
      for sideLength in $sideLengths; do
        for messageCount in $messageCounts; do
          for timeStepCount in $timeStepCounts; do
            outfile=${nodeCount}_${tasksPerNode}_${threadCount}_${sideLength}_${messageCount}_${timeStepCount}_json.time
            sbatch -N $nodeCount --cpus-per-task $threadCount --ntasks-per-node $tasksPerNode -o $outfile scaling-json.sh $nodeCount $tasksPerNode $threadCount $sideLength $messageCount $timeStepCount
          done
        done
      done
    done
